...
import { useGameTransition } from "./use-game-transition";
...
export default function GameRoom() {
  ...
  const { game, switching } = transition;
  ...
  function roll() {
    if (busyRef.current || switching || position === 24) return;
  ...
  function switchGame(next: "board" | "wheel") {
    if (busyRef.current || editing || next === transition.selected) return;
    if (menu.current) menu.current.open = false;
    setPreview(null); transition.request(next); setEditing(false); setNote("");
  }
  useEffect(() => {
    setAnnouncement(game === "board" ? "Puppy Steps." : "The Wheel. Spin to choose.");
  }, [game]);

  function resetBoard() { if (!busyRef.current && !switching) { setPreview(null); setPosition(0); setDice(1); setEditing(false); setAnnouncement("Back at the start."); } }
  ...
  return <main className="room immersive-room" data-game={transition.selected} data-scene-phase={transition.phase} data-scene-direction={transition.direction}>
...
<div className="game-switch visible-game-switch" aria-label="Choose a game"><button aria-pressed={transition.selected === "board"} disabled={busy || editing} onClick={() => switchGame("board")}>Puppy Steps</button><button aria-pressed={transition.selected === "wheel"} disabled={busy || editing} onClick={() => switchGame("wheel")}>The Wheel</button></div>
    ...
    <GameLibrary config={{ spaces, options }} disabled={busy || editing || switching} dirty={dirty} onAdmin={setCanEdit} onSaved={() => setDirty(false)} onLoad={config => { setSpaces(config.spaces); setOptions(config.options); setDraft(config.spaces[0]); setOptionDraft(config.options); setPosition(0); setPreview(null); setSelected(0); setAngle(0); setWinner(null); setEditing(false); setDirty(false); setNote(""); }} />
          {canEdit && game === "wheel" && <button className="game-secondary" disabled={busy || switching} onClick={() => { setOptionDraft(options.map(o => ({ ...o }))); setEditing(true); }}>Edit choices</button>}
          <Link href="/" className="game-link">Leave ↗</Link>
        </div>
      </details>
    </header>
    <div className="immersive-stage scene-stack" aria-busy={switching}>
      <div className="scene-curtain" aria-hidden="true" onAnimationEnd={event => {
        if (event.target !== event.currentTarget) return;
        if (event.animationName === "scene-cover") transition.advance("cover");
        if (event.animationName === "scene-reveal") transition.advance("reveal");
      }}><span className="scene-seal">S</span></div>
      <section className={`scene-layer photo-board ${game === "board" ? "scene-active" : ""}`} aria-label="Puppy Steps board" aria-hidden={game !== "board"} inert={game !== "board" || switching}>
      ...
      <section className={`scene-layer immersive-wheel ${game === "wheel" ? "scene-active" : ""}`} aria-label="The Wheel" aria-hidden={game !== "wheel"} inert={game !== "wheel" || switching}><div className="wheel-stage"><div className="wheel-pointer" aria-hidden="true" /><div className="prize-wheel" style={{ background: `conic-gradient(${gradient})`, transform: `rotate(${angle}deg)` }} aria-hidden="true">{options.map((option, i) => <div className="wheel-label" key={i} style={{ transform: `rotate(${slices[i].center}deg)` }}><span>{slices[i].percent >= 8 ? option.label.slice(0, 20) : i + 1}</span></div>)}</div><div className="wheel-center" aria-hidden="true">S<span>✧</span></div></div></section>
    </div>
    <section className="play-dock" aria-label="Game controls" inert={switching}>
      ...
      <div className="dock-actions">
        ...
        <button className="game-primary roll-button" disabled={busy || switching} onClick={game === "board" ? position === 24 ? resetBoard : roll : spin}>{busy ? "…" : game === "board" ? position === 24 ? "Play again" : "Roll" : "Spin"}<span aria-hidden="true">↗</span></button>
        {game === "board" && <button className="reset-button" aria-label="Start again" title="Start again" disabled={busy || switching} onClick={resetBoard}>↺</button>}
      </div>
