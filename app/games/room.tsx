...
export default function GameRoom() {
  ...
  useEffect(() => { if (editing && game === "board") editorTitle.current?.focus(); }, [editing, selected, game]);
  ...
  function roll() {
    ...
    setEditing(false); setRunning(true); setNote("");
  ...
  function spin() {
    ...
    setEditing(false); setRunning(true); setWinner(null); setNote("");
  ...
  function switchGame(next: "board" | "wheel") {
    if (busyRef.current) return;
    setGame(next); setEditing(false); setNote(""); setAnnouncement(next === "board" ? "Roll to begin." : "Spin to choose.");
  }
    ...
    <div className={`game-layout ${game === "board" ? "board-layout" : ""}`}>
      ...
      <aside className="game-panel">
        {editing && canEdit ? <form onSubmit={event => { event.preventDefault(); if (game === "board") { if (!draft.title.trim() || !draft.content.trim()) { setNote("Add a title and instruction."); return; } save(spaces.map((s, i) => i === selected ? { title: draft.title.trim(), content: draft.content.trim() } : s), options); } else { if (optionDraft.some(o => !o.label.trim() || !Number.isInteger(o.weight) || o.weight < 1 || o.weight > 100)) { setNote("Add a name and a whole-number weight from 1 to 100."); return; } save(spaces, optionDraft.map(o => ({ label: o.label.trim(), weight: o.weight }))); setWinner(null); setAngle(0); } setEditing(false); }}>
          ...
          <span className="room-kicker">{game === "board" ? "YOUR NEXT MOVE" : "A TURN OF FATE"}</span><h2>{game === "board" ? "Follow the paws." : "Round we go."}</h2>
          {game === "board" ? <><div className={`die ${busy ? "rolling" : ""}`} aria-label={`Dice: ${dice}`}><span aria-hidden="true">{dieFaces[dice - 1]}</span></div><div className="position-meta"><span>YOUR POSITION</span><strong>{String(position).padStart(2, "0")} <small>/ 24</small></strong></div><div className="landing-instruction"><span className="room-kicker">{position === 24 ? "FINISH" : "CURRENT SPACE"}</span><h3>{spaces[position].title}</h3><p>{spaces[position].content}</p></div><button className="game-primary" disabled={busy || position === 24} onClick={roll}>{busy ? "On the move…" : position === 24 ? "You made it!" : "Roll the dice"}<span>↗</span></button><div className="panel-actions"><button className="game-link" disabled={busy} onClick={resetBoard}>↺ Start again</button>{canEdit && <button className="game-link" disabled={busy} onClick={() => selectSpace(position)}>Edit space</button>}</div><p className="field-help">Move 1–6 spaces. Rolls beyond the finish stop at the final space.</p></> : <><div className="wheel-result"><span className="room-kicker">{busy ? "THE WHEEL IS TURNING" : winner ? "YOUR RESULT" : "WHAT WILL IT BE?"}</span><h3>{busy ? "A little suspense…" : winner || "Take a chance."}</h3></div><button className="game-primary" disabled={busy} onClick={spin}>{busy ? "Spinning…" : "Spin the wheel"}<span>↗</span></button>{canEdit && <button className="game-secondary" disabled={busy} onClick={() => { setOptionDraft([...options]); setEditing(true); setNote(""); }}>Edit choices</button>}<div className="choice-key">{options.map((o, i) => <div key={i}><span style={{ background: colors[i] }} />{i + 1}. {o.label}<small>{slices[i].percent.toFixed(1)}%</small></div>)}</div></>}
        </>}
        {note && <p className="save-note" role="status">{note}</p>}
        <p className="storage-note">{canEdit ? "Save your game to keep edits across devices." : "Games created by Skye."}</p>
      </aside>
    </div>
    <p className="sr-only" role="status" aria-live="polite" aria-atomic="true">{announcement}</p>
    <footer className="room-footer"><span>© {new Date().getFullYear()} PRINCESS SKYE</span><span>JUST FOR PLAY · 18+</span></footer>
  </main>;
}
